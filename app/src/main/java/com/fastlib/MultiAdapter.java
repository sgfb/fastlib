...
public abstract class MultiAdapter extends BaseAdapter{
    ...
    private List<ItemGroup> mData;  //类型与索引对齐 类型0对于List(0)
     ...
     * @return
     */
    private ItemGroup findGroupByType(int type){
        if(mData==null) return null;
        checkGroup(type);
        for(ItemGroup group:mData)
            if(type==group.type)
     ...
     * @return
     */
    public ItemGroup getItemGroupByType(int type){
        return mData.get(type);
    }

...
//    private Point[] mOldDownPosition=new Point[]{new Point(),new Point()};
    public static final int SCALE_ANIMATOR_DURATION = 200; //图片缩放动画时间
    public static final float FLING_DAMPING_FACTOR = 0.9f; //惯性动画衰减参数
    private static final float MAX_SCALE = 4f; //图片最大放大尺寸
    public static final int PINCH_MODE_FREE = 0; //手势状态：自由状态
    public static final int PINCH_MODE_SCROLL = 1; //手势状态：单指滚动状态
    public static final int PINCH_MODE_SCALE = 2; //手势状态：多指缩放状态
    private Matrix mOuterMatrix = new Matrix(); //外层变换矩阵，如果是单位矩阵，那么图片是fit center状态
    private RectF mMask; //矩形遮罩
    //手势状态，值为PINCH_MODE_FREE，PINCH_MODE_SCROLL，PINCH_MODE_SCALE
    private int mPinchMode = PINCH_MODE_FREE;

    //点击，双击，长按，滑动等手势处理
    private GestureDetector mGestureDetector = new GestureDetector(this.getContext(), new GestureDetector.SimpleOnGestureListener() {
    ...
    private void doubleTap(float x, float y) {
        ...
        RectF testBound = new RectF(0, 0,getWidth(),getHeight());
...
//     * @param event
//     */
//    private void oneFingerEvent(MotionEvent event){
//        if(event.getAction()==MotionEvent.ACTION_DOWN)
//            mOldDownPosition[0].set((int)event.getX(),(int)event.getY());
//        else if(event.getAction()==MotionEvent.ACTION_MOVE){
//            //如果diff为正数，右移否则为左移,0除外
//            float diffX= event.getX()-mOldDownPosition[0].x;
//            float diffY=event.getY()-mOldDownPosition[0].y;
//            float fixX=getTranslationX()+diffX;
//            float fixY=getTranslationY()+diffY;
//
//            if(fixX<0) fixX=0;
//            if(fixY<0) fixY=0;
//        }
